...
if __name__ == "__main__":
    ...
    print("Read data...")
    test_data = []
    with open(sys.argv[1], "r", encoding='utf-8-sig') as input_data:
    ...
    print(test_data[:10])    
    label = []
    with open(sys.argv[2], "r", encoding='utf-8-sig') as input_data:
        for sentence in input_data:
            sentence = sentence.replace("\n",'')
            label.append(int(sentence))
        input_data.close()
        label = to_categorical(label, num_classes=num_label)
    print(label[:10])
    print("\nWord embedding...")
    ...
    print("\nEvaluate...")
    score, acc = model.evaluate(word_vectors, label, batch_size=1)
    ...
    print(confusion_matrix(y_test.argmax(axis=1), label.argmax(axis=1)))